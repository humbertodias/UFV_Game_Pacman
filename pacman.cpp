...
int intencao = 0; // intenção de movimento atual
...
char MAPA[24][24] = {
    "11111111111111111111111", "12222222222122222222221",
    "12111211112121111211121", "12111211112121111211121",
    "12222222222222222222221", "12111212111111121211121",
    "12222212222122221222221", "11111211110101111211111",
    "11111210000000001211111", "11111210111011101211111",
    "00000000111011100000000", "11111210111011101211111",
    "11111210111111101211111", "11111210000000001211111",
    "11111210111111101211111", "12222222222122222222221",
    "12111211112121111211121", "12221222222022222212221",
    "11121212111111121212111", "12222212222122221222221",
    "12111111112121111111121", "12222222222222222222221",
...
ALLEGRO_SAMPLE *waka = NULL;
ALLEGRO_SAMPLE *win = NULL;
ALLEGRO_SAMPLE *death = NULL;
ALLEGRO_SAMPLE *beggining = NULL;
ALLEGRO_FONT *fonte = NULL;

int i = 17, j = 11;   // Posição do PacMan
int r = 8, t = 11;    // Posição do Azul
int aX = 9, aY = 11;  // Posição do fantasma Amarelo
int vX = 10, vY = 11; // Posição do fantasma Verde
int g = 11, h = 11;   // Posição do Blinky
...
// Auxiliares de "memoria" dos personagens, indicam o último movimento realizado
int lastRandomPos = -1;
...
int ulx = g; // Ultimas coordenadas do Blinky, usado pra morte do PacMan e para
             // a movimentação dele mesmo
int uly = h;

// Ultimas coordenadas de 3 fantasmas e do PacMan (para realizar a morte do
// PacMan)
int lastamarelox = aX, lastamareloy = aY, lastazulx = r, lastazuly = t,
...
void blinkyMove(char M[][24], int &x, int &y, int &bX, int &bY) {
  ...
  if ((x == 8) && (y == 11) &&
      (j <= y)) { // ifs para o blinky nao entrar na zona de nascimento
    ulx = x;
    uly = y;
    y--;
    bX = y * q;
  ...
  vai[0].dist = distancia(x + 1, y, i, j);
  vai[0].x = x + 1;
  vai[0].y = y;
  vai[0].seta = 'S';
  vai[1].dist = distancia(x, y - 1, i, j);
  vai[1].x = x;
  vai[1].y = y - 1;
  vai[1].seta = 'A';
  vai[2].dist = distancia(x - 1, y, i, j);
  vai[2].x = x - 1;
  vai[2].y = y;
  vai[2].seta = 'W';
  vai[3].dist = distancia(x, y + 1, i, j);
  vai[3].x = x;
  vai[3].y = y + 1;
  vai[3].seta = 'D';
  ...
  int which = 1000;
  for (int c = 0; c < 4;
       c++) { // vendo qual distancia é menor e ao mesmo tempo acessivel
  ...
  if (which == 0) { // indo pela menor distância
    x++;
    bY = x * q;
  }
...
void semiSmart(char M[][24], int &x, int &y, int &gposX, int &gposY,
               int &lastThisPos) {

  // Lembrar que aqui o eixo X e controlado pelas colunas (y) e o Y pelas linhas
  // (x) Para nao alterarmos o valor de x e y nas condicionais, pois sao
  // recebidos por referencia
  int auxX = x;
  int auxY = y;
  ...
  if (i > x && M[auxX + 1][y] != '1') {
    if (lastThisPos == 0)
      return; // Se o último movimento foi o contrário do atual, nao fazer nada
    lastThisPos = 1;
    x++;
    gposY = x * q;
  }
...
void randomMove(char M[][24], int &x, int &y, int &gposX, int &gposY,
                int phantom) {
  ...
  // Para que os fantasmas não escolham fazer o mesmo movimento "aleatorio",
  // sorteia-se outro numero com base no horário do sistema.
  if (phantom == 1)
    srand(time(NULL));
  randomIndex = rand() % 4;
...
int inicializa() {
  ...
  win = al_load_sample("audio/win.wav");
  death = al_load_sample("audio/wasted.wav");

  beggining = al_load_sample("audio/beggining.wav");
  if (!win) {
  ...
  waka = al_load_sample("audio/waka.wav"); // musica que sera carregada

  if (!waka) {
    printf("Audio clip waka not loaded!\n");
    return -1;
  }
  ...
  mapa = al_load_bitmap("images/map.bmp");
  if (!mapa) {
  ...
  mapa2 = al_load_bitmap("images/map2.bmp");
  if (!mapa2) {
  ...
  perdeu = al_load_bitmap("images/mapmorte.bmp");
  if (!perdeu) {
  ...
  pac_up = al_load_bitmap("images/pac_up.png");
  pac_down = al_load_bitmap("images/pac_down.png");
  pac_left = al_load_bitmap("images/pac_left.png");
  pac_right = al_load_bitmap("images/pac_right.png");
  shutup = al_load_bitmap("images/shutup.png");
  blinky = al_load_bitmap("images/blinky.png");
  azul = al_load_bitmap("images/azul.png");
  ghostAmarelo = al_load_bitmap("images/amarelo.png");
  ghostVerde = al_load_bitmap("images/verde.png");
  ...
  bolas = al_load_bitmap("images/bolas.png");
  if (!bolas) {
  ...
  fonte = al_load_font("fonts/ocrext.ttf", 28, 0);
  if (!fonte) {
    al_destroy_display(display);
...
int main(int argc, char **argv) {
  ...
  while (!sair) {
    ...
    if (ev.type == ALLEGRO_EVENT_TIMER) {
      ...
      // movimentação baseada no jogo original
      if (intencao == 4) {
        if (MAPA[i - 1][j] != '1') {
          pacman = pac_up;
          lastmouth = 2;
      ...
      if (sim % 2 == 0) {
        aux = pacman;
        pacman = shutup; // Se a variavel sim for par, redraw o pacman com boca
                         // fechada
        redraw = true;
      } else {
        pacman = aux;
    ...
    al_draw_textf(fonte, al_map_rgb(200, 200, 200), 0, 505, 0, "Score: %d",
                  pontos);
    ...
    if (redraw && al_is_event_queue_empty(event_queue)) {
      ...
      if (contador >= 210) // por último o blinky
    ...
    if (playwaka) {
      al_play_sample(waka, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_LOOP, NULL);
      playwaka = false;
    }
    ...
    // Código para o pacman morrer quando ocupar a mesma posição que um fantasma
    if ((g == i && h == j) || (i == r && j == t) || (i == aX && j == aY) ||
        (i == vX && j == vY)) {
      gameover = true;
    }

    // Código para o pacman morrer quando ele e algum fantasma trocarem de
    // posições
    else if ((lastpacmanx == g && lastpacmany == h && i == ulx && j == uly) ||
             (lastpacmanx == r && lastpacmany == t && i == lastazulx &&
              j == lastazuly) ||
             (lastpacmanx == aX && lastpacmany == aY && i == lastamarelox &&
              j == lastamareloy) ||
             (lastpacmanx == vX && lastpacmany == vY && i == lastverdex &&
              j == lastverdey)) {
      gameover = true;
    }
